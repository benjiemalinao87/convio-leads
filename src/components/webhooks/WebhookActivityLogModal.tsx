...
interface WebhookActivityLogModalProps {
  webhookId: string;
  webhookName: string;
  open: boolean;
...
export function WebhookActivityLogModal({ 
  webhookId, 
  webhookName,
  open, 
  onOpenChange 
}: WebhookActivityLogModalProps) {
  ...
  const fetchRecentLeads = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`${API_BASE}/leads?webhook_id=${webhookId}&limit=20`);
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-4">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm font-medium mb-1">No leads received yet</p>
              <p className="text-xs">Activity will appear here when leads are received</p>
            </div>
          ) : (
